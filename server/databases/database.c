...
int
database_init (char *config_file)
{
    database_connection_s connect = {0};

    data_result_s result
      = data_parse_files (config_file, configs, sizeof (configs) / sizeof (data_keys_s));
...
database_verify_integrity ()
{
    ...
    if (MYSQL_SUCCESS == mysql_query (sql_connection, DATABASE_CREATE_DATABASE))
    {
...
int
database_create_tables (char *statement, char *table)
{
...
int
database_check_tables (char *statment, char *table)
{
...
database_table_verify (
  char *check_table_query, char *create_table_query, char *table_name, int (*binds_setup) (MYSQL *))
{
    if (
      database_check_tables (check_table_query, table_name) != MYSQL_SUCCESS
      && database_create_tables (create_table_query, table_name) != MYSQL_SUCCESS)
    {
        return (FAILED);
    }
...
int
database_table_call1 (MYSQL_STMT *stmt, string_s query, MYSQL_BIND *bind, int bind_count)
{
...
database_table_insert (
  int (*database_function) (MYSQL_STMT *), string_s query, MYSQL_BIND *bind, size_t count)
{
...
int
database_table_row_exists (MYSQL_STMT *stmt)
{
...
int
database_table_stmt (string_s query, MYSQL_BIND *bind_in, uint bind_in_count)
{
    MYSQL_STMT *stmt = mysql_stmt_init (sql_connection);
    if (stmt == NULL)
    {
...
int
database_table_query (
  int (*database_function) (MYSQL_STMT *),
  string_s query,
  MYSQL_BIND *bind_in,
  uint bind_in_count,
  MYSQL_BIND *bind_out)
...
database_bind_setup (MYSQL *mysql, char *select_query)
{
    ...
    while (column = mysql_fetch_field (result), column != NULL)
    {
        ...
        {
            table_binds.table_name = string_new_copy (column->table, column->table_length);
        }
        ...
        output_handle (
          ...
          table_binds.table_name.address,
          field.name.length,
          field.name.address);
    }
...
database_bind_select_copy (database_table_bind_s source, string_s *columns, int count)
{
    ...
    {
        string_s string = columns[i];
        hash_input_u search = (hash_input_u){.address = string.address};

        hash_table_bucket_s bucket = hash_table_get (source.hash_table, search, string.length);
...
void
database_bind_allocate (database_table_bind_s *bind, size_t columns)
{
...
size_t
database_bind_column_index (database_table_bind_s bind_table, string_s column_name)
{
    hash_input_u search = (hash_input_u){.address = column_name.address};

    hash_table_bucket_s bucket = hash_table_get (bind_table.hash_table, search, column_name.length);
...
void
database_bind_clean (database_table_bind_s bind_table)
{
...
void
database_bind_free (database_table_bind_s bind)
{
...
hash_table_s
database_bind_maketable (database_table_bind_s *bind_table)
{
...
database_bind_set_flags (size_t flags)
{
...
database_bind_field (MYSQL_FIELD *field, database_bind_fields_s *src)
{
    ...
    string_s buffer = {.address = address, .length = buffer_length};
    column.buffer = buffer;
    string_s name = {.address = buffer.address + buffer_length, .length = name_length};
    column.name = name;
...
void
database_bind_link_fields (database_table_bind_s *table)
{
...
int
database_bind_add_data (database_table_bind_s bind_table, string_s column_name, string_s data)
{
