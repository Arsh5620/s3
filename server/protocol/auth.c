...
{
	boolean user	= dbp_attrib_contains(request->header_table
		, DBP_ATTRIB_USERNAME);
	boolean secret	= dbp_attrib_contains(request->header_table
		, DBP_ATTRIB_SECRET);
...
int dbp_auth_query(dbp_request_s *request)
{
	...
	if(data_get_and_convert(request->header_list, request->header_table
		, DBP_ATTRIB_USERNAME
		, DATA_TYPE_STRING_S, (char*)&username, sizeof(string_s)) != SUCCESS)
	{
		return (DBP_RESPONSE_SERVER_ERROR_NOAUTH);
	}
	if(data_get_and_convert(request->header_list, request->header_table
		, DBP_ATTRIB_SECRET
		, DATA_TYPE_STRING_S, (char*)&secret, sizeof(string_s)) != SUCCESS)
	{
		return (DBP_RESPONSE_SERVER_ERROR_NOAUTH);
	}
	...
	string_s columns[] = {
		AUTH_COLUMN_USERNAME
		, AUTH_COLUMN_SECRET
	};
	database_table_bind_s bind_copy	= 
		database_bind_select_copy(auth_table_binds, columns, 2);

	if (database_bind_add_data(auth_table_binds
		, AUTH_COLUMN_USERNAME, username) == MYSQL_ERROR)
	{
	...
	if (database_bind_add_data(auth_table_binds
		, AUTH_COLUMN_SECRET, sha_hash_s) == MYSQL_ERROR)
	{
		return(FILEMGMT_SQL_COULD_NOT_BIND);
	}
