...
ulong dbp_request_read_headers(dbp_protocol_s protocol, dbp_request_s *request)
{
    ...
    {
        output_handle(OUTPUT_HANDLE_LOGS, LOGGER_LEVEL_ERROR, PROTOCOL_ABORTED_CORRUPTION, request->header_info.magic);
        return (DBP_RESPONSE_CORRUPTED_PACKET);
    }

    network_data_s header_raw = network_read_stream(&protocol.connection, request->header_info.header_length);
    ...
    deserializer_t deserializer = deserializer_init(header_raw.data_address, header_raw.data_length);
    ...
    for (int i = 0; i < source_list.count; ++i)
    {
        ...
        if (result.key == 0 || result.key_size == 0 || result.value_2 == NULL)
        {
            output_handle(OUTPUT_HANDLE_LOGS, LOGGER_LEVEL_WARN,
                          "Binary deserializer: A value was found without "
                          "a key, ignored. If you did not intend for this to happen, "
                          "then this could be a result of misalignment/error/corruption/bug."
...
int dbp_request_read_action(dbp_request_s *request)
{
    ...
    if (action.strlen == pair.key_length && memcmp(pair.key, action.string, action.strlen) == 0)
    {
        ...
        actionval = binary_search(actions, sizeof(data_keys_s), DBP_ACTIONS_COUNT, pair.value, pair.value_length, data_key_compare);
    }
...
dbp_header_s inline dbp_header_parse8(size_t magic)
{
    ...
    dbp_header_s header = {
        .header_length = (((magic & 0x00FF000000000000) >> (6 * 8)) * 16), .data_length = (magic & 0x0000FFFFFFFFFFFF), .magic = ((magic & 0xFF00000000000000) >> (7 * 8))};
