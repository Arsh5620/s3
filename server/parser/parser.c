...
void
parser_release_list (my_list_s list)
{
...
void
parser_push_copy (my_list_s *list, key_value_pair_s pair)
{
...
parser_parse_file (FILE *file)
{
    ...
    do
    {
        ...
        if (status.status == PARSER_EOF)
        {
            ...
            if (status.base_index == 0 && reader.readlength == reader.maxlength)
            {
            ...
            if (bytes_move > 0)
            {
                memmove (reader.buffer, reader.buffer + status.base_index, bytes_move);
            }
    ...
    } while ((reader.eof == 0 || reader.readlength > 0) && status.err_no == 0);
...
static char *warnings[] = {"some data was found after entire statement was read,"
                           " consider adding quotations.\n"};

void
parser_print_lineinfo (lexer_s lexer, lexer_status_s status, char error, char *message)
{
    ...
    output_handle (
      ...
      rest,
      lexer.buffer + status.base_index + stat,
      stat,
      0,
      &c,
      message);
...
void
parser_print_err (lexer_s lexer, lexer_status_s status)
{
...
void
parser_print_warn (lexer_s lexer, lexer_status_s status)
{
...
void
parser_print_status (lexer_s lexer, lexer_status_s status)
{
    if (status.warnno)
    {
...
key_value_pair_s
parser_parse_next (lexer_s *lexer, lexer_status_s *lstatus)
{
    ...
    do
    {
        ...
        case TOKEN_CARRIAGERETURN:
        {
            ...
            // flag an error
            if (status != PARSER_WAIT_KEY && status != PARSER_WAIT_NONE)
            {
                pair.is_valid = FALSE;
        ...
        case TOKEN_NONE:
        {
            if (status != PARSER_WAIT_NONE && status != PARSER_WAIT_END)
            {
                pair.is_valid = FALSE;
            }
            lstatus->status = PARSER_EOF;
    ...
    } while (token.token_type != TOKEN_NONE && pair.is_valid == TRUE && status != PARSER_WAIT_END);
