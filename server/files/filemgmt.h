...
#define FILEMGMT_QUERY_EXISTS  \
	"SELECT * FROM " FILEMGMT_TABLE_NAME \
	" WHERE file_name = ?"

#define FILEMGMT_QUERY_DELETE \
	"DELETE FROM " FILEMGMT_TABLE_NAME \
	" WHERE file_name = ? LIMIT 1;"

#define FILEMGMT_QUERY_INSERT \
	"INSERT INTO " FILEMGMT_TABLE_NAME \
	" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
#define FILEMGMT_TABLE_CREATE \
	DATABASE_CREATE_TABLE(FILEMGMT_TABLE_NAME, \
...
#define FILEMGMT_BIND \
	DATABASE_COMMON_BIND_STMT(FILEMGMT_TABLE_NAME)

#define FILEMGMT_TABLE_CHECK \
