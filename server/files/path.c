...
my_list_s path_lex(string_s path, boolean *is_absolute)
{
	...
	char *index_p	= path.address
		, *length_p	= (path.address + path.length);
...
file_path_s path_parse(string_s path)
{
	...
	for (ulong i=0; i<path_list.count; ++i)
	{
		...
		if (node->length == 1 && *(node->buffer + node->index) == '.')
		{
			...
			&& memcmp(node->buffer + node->index, "..", 2) == 0)
		{
...
string_s path_construct(my_list_s path_list)
{
	...
	{
		file_path_node_s path	= 
			*(file_path_node_s*) my_list_get(path_list, i);
