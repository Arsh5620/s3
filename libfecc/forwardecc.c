...
int cmp_blocks_equal(ff_t *block1, ff_t *block2, int size)
{
	return (memcmp(block1, block2, size) == 0);
}

void init_message(poly_s poly)
{
...
void corrupt_message1(poly_s poly, int error_count)
{
...
void corrupt_message2(poly_s poly, int start_at, int error_count)
{
...
int main()
{
	...
	{	
		for (size_t ecase = 0; ecase < 2; ecase++)
		{
			...
			memcpy(decode.message_in_buffer.memory
			...
			{	
				rs_decode(&decode);
				if (!cmp_blocks_equal(encode.message_in_buffer.memory
			...
			printf("Decoding at n = %d, and k = %d, case type: %s"
