 ...
 */
FECC_INLINE
void ff_table_ssemap(ff_t *memory, ff_t val, short irr_p)
{
...
ff_table_s ff_table_new(short irr_p)
{
	...
	table.buffer_alloc	= aligned_alloc(MEMORY_ALIGNMENT
		, ALLOCATE_ALIGNMENT(total_allocation * sizeof(ff_t)));

	// We only do one big memory allocation and distribute it across
	// different tables here. 
	TABLE_INDEX(table.multiply_table, table.buffer_alloc
		, memory_index, FF_SIZE * FF_SIZE);
	TABLE_INDEX(table.logs, table.buffer_alloc
		, memory_index, FF_SIZE);
	TABLE_INDEX(table.exponents, table.buffer_alloc
		, memory_index, FF_SIZE);
	TABLE_INDEX(table.ssemap, table.buffer_alloc
		, memory_index, (FF_SIZE * SIMD_VECTOR_SIZE * 2));
...
void ff_table_free(ff_table_s table)
{
...
FECC_INLINE
ff_t ff_multiply_lut(ff_t *full_table, ff_t x, ff_t y)
{
	/*
 ...
 */
FECC_INLINE
__m128i ff_multiply_lut_sse(ff_t *ssemap_table, __m128i vector, ff_t scalar)
{
	/**
	...
	__m128i *ssemap	= 
		(__m128i*)(ssemap_table + (scalar * SIMD_VECTOR_SIZE * 2));
